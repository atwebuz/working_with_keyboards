...
from keyboards.default.start_menu_keyboard import menu_register
...
@dp.message_handler(text_contains='Yopish')
async def select_category(message: Message):
    await message.answer(' Yopish', reply_markup=ReplyKeyboardRemove())
...
async def buying_book(call: CallbackQuery, callback_data: dict):
    logging.info(f'{course_callback=}')
    await call.message.delete()
    await call.message.answer(f"C++ qabul qilindi\nRaqam va Lokatsa yuboring", reply_markup=menu_register)

    # await call.answer('C++ qabul qilindi', cache_time=60, show_alert=True, reply_markup=menu_register)
